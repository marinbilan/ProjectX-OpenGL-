...
namespace Shaders
{
class ShaderPTN1 : public ShadersIf::ShadersIf
{
...
public:
	...
	ShaderPTN1(GLfloat projMatrixWidth, GLfloat projMatrixHeight);
	...
	// OPERATORs
	void printINFO();
	friend std::ostream& operator<<(std::ostream& output, ShaderPTN1& info)
	{
		output << "" << std::endl;
		...
		output << "      specularMapID      = " << info.specularMapID << std::endl;
		output << "" << std::endl;
		return output;
	}
private:
